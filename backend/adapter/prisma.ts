...
import { DatabaseAdapter, FindByIdOptions } from "../types/db_adapter";
import {
  QueryOptions,
  Filter,
  FilterGroup,
...
export class PrismaAdapter<T> implements DatabaseAdapter<T> {
  ...
  private buildPrismaOptions(options?: QueryOptions<T>): any {
    ...
    if (options.select && options.include) {
      throw new Error(
        "QueryOptions cannot contain both 'select' and 'include' for Prisma queries.",
      );
    }
    ...
    if (options.select) {
      prismaOptions.select = options.select.reduce(
